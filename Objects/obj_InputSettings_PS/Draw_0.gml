...
var gp=gamepad_get_device_count();
        key_down=gamepad_button_check(4,MenuDown);
        key_up=gamepad_button_check(4,MenuUp);
        key_select=gamepad_button_check(4,MenuSelect);
        key_quit=gamepad_button_check(4,MenuQuit);
...
for (var i=0; i<key_num;i++;)
    {
        textDisplay=string(inputName[i]) + " : [ " + string(scr_recognizeInputP(inputArray[i])) + " ]";
        if i=optionSelected{
